        ...
        <v-container style="padding: 0;" :hidden="loading">
          ...
          <v-window v-model="tab">
            <v-window-item value="t1">
              ...
              <v-row>
                <v-col cols="12" sm="6" md="4">
                  <v-text-field v-model.number="Volume" type="number" min="0" :label="$t('stats.volume')" suffix="GiB"
              ...
              <v-row v-if="id > 0">
                <v-col cols="12" sm="6" md="4" class="d-flex flex-column">
                  ...
                  <v-progress-linear v-model="percent" :color="percentColor" v-if="client.volume > 0" bottom>
                  </v-progress-linear>
                </v-col>
                <v-col cols="12" sm="6" md="4">
              ...
              <v-row>
                <v-col>
                  <v-select v-model="clientInbounds" :items="inboundTags" :label="$t('client.inboundTags')" clearable
            ...
            <v-window-item value="t2">
              <v-row>
                <v-col cols="12" sm="6" md="4">
                  <v-btn variant="tonal" @click="shuffle()">{{ $t('reset') + ' - ' + $t('all') }}<v-icon
              ...
              <v-row v-for="key in Object.keys(clientConfig)">
                ...
                <v-col>
                  <v-text-field v-if="clientConfig[key].password != undefined" label="Password"
                    v-model="clientConfig[key].password" hide-details>
                  </v-text-field>
                  <v-text-field v-if="clientConfig[key].uuid != undefined" label="UUID" v-model="clientConfig[key].uuid"
                    hide-details>
                  </v-text-field>
                  <v-text-field v-if="key == 'vless'" label="Flow" v-model="clientConfig[key].flow" hide-details>
                  </v-text-field>
                  <v-select v-if="key == 'vless' && clientConfig[key].flow && clientConfig[key].flow !== ''"
                    label="Flow Inbound Tags" :placeholder="$t('client.vlessFlowTagsPlaceholder')"
                    v-model="clientConfig[key].flow_inbound_tags" :items="vlessInboundTags" clearable multiple chips
                    hide-details>
                  </v-select>
                  <v-text-field v-if="key == 'hysteria'" label="Auth" v-model="clientConfig[key].auth_str" hide-details>
                  </v-text-field>
                </v-col>
              </v-row>
            </v-window-item>
            <v-window-item value="t3">
              ...
              <v-row>
                <v-col>
                  <v-btn color="primary" @click="extLinks.push({ type: 'external', uri: '' })">{{ $t('actions.add') }}
                    {{
                      $t('client.external') }}</v-btn>
                </v-col>
              </v-row>
              <v-row v-for="(lnk, index) in extLinks">
                <v-col>
                  <v-text-field dir="ltr" :label="$t('client.external') + ' ' + (index + 1)" append-icon="mdi-delete"
                    @click:append="extLinks.splice(index, 1)" placeholder="<protocol>://<data>" v-model="lnk.uri" />
                </v-col>
              </v-row>
              <v-row>
                <v-col>
              ...
              <v-row v-for="(lnk, index) in subLinks">
                <v-col>
                  <v-text-field dir="ltr" :label="$t('client.sub') + ' ' + (index + 1)" append-icon="mdi-delete"
                    @click:append="subLinks.splice(index, 1)" placeholder="http[s]://<domain>[:]<port>/<path>"
                    v-model="lnk.uri" />
                </v-col>
              </v-row>
            </v-window-item>
...
export default {
  ...
  computed: {
    ...
    vlessInboundTags() {
      const store = Data();
      return getVlessInboundTags(store.inbounds, this.client.inbounds, true);
    }
  },
  watch: {
    visible(newValue) {
